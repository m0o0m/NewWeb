    ...
    public class CustomerServCenterDAL
    {
        ...
        internal static IEnumerable<CustomerServCenter> GetList(int GUUserID)
        {
            using (var cn = new MySqlConnection(sqlconnectionString))
            {
                ...
                IEnumerable<CustomerServCenter> csc = cn.Query<CustomerServCenter>(@"select * from CustomerServCenter a, (SELECT CSCMainID FROM CustomerServCenter where GUUserID = @GUUserID and CSCSubId=0 limit 0, 20) b where a.CSCMainID = b.CSCMainID or a.CSCSubId = b.CSCMainID", new { GUUserID = GUUserID });
