...
namespace GL.Data.BLL
{
    public class ExpRecordBLL
    {
        ...
        public static PagedList<ItemRecord> GetItemRecordList(GameRecordView model)
        {
            ...
            pq.RecordCount = PagedListDAL<ItemRecord>.GetRecordCount(string.Format(@"select count(0) from record.BG_ItemRecord 
...
from (
    select * from record.BG_ItemRecord where CreateTime between '{2}' and '{3}' and UserID = case when {4} = 0 then UserID else {4} end 
      and ItemID = case when {5} = 0 then ItemID else {5} end order by CreateTime desc limit {0}, {1}
) as aa 
  join Role on aa.UserID = Role.ID left join record.S_Desc c on aa.ChangeType = c.Type
  left join 515game.S_Template t on aa.ItemID = t.TempalteID and t.TemplateType = 0 ;
"
        ...
        public static PagedList<ExpRecord> GetListByPage(GameRecordView model)
        {
            ...
            {
                pq.RecordCount = PagedListDAL<ExpRecord>.GetRecordCount(string.Format(@"select count(0) from record.BG_ExpRecord where CreateTime between '{0}' and '{1}' and UserID in (select ID from Role where ID = '{2}' or Account='{2}' or NickName='{2}')", model.StartDate, model.ExpirationDate, model.SearchExt), sqlconnectionString);

                pq.Sql = string.Format(@"select aa.*, Role.NickName as UserName from (select * from record.BG_ExpRecord where CreateTime between '{2}' and '{3}' and UserID in (select ID from Role where ID = '{4}' or Account='{4}' or NickName='{4}') order by CreateTime desc limit {0}, {1}) as aa inner join Role on aa.UserID = Role.ID", pq.StartRowNumber, pq.PageSize, model.StartDate, model.ExpirationDate, model.SearchExt);

            }
            else
            {
                pq.RecordCount = PagedListDAL<ExpRecord>.GetRecordCount(string.Format(@"select count(0) from record.BG_ExpRecord where CreateTime between '{0}' and '{1}'", model.StartDate, model.ExpirationDate), sqlconnectionString);
                pq.Sql = string.Format(@"select aa.*, Role.NickName as UserName from (select * from record.BG_ExpRecord where CreateTime between '{2}' and '{3}' order by CreateTime desc limit {0}, {1}) as aa inner join Role on aa.UserID = Role.ID", pq.StartRowNumber, pq.PageSize, model.StartDate, model.ExpirationDate);
            }
