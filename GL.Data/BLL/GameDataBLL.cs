...
namespace GL.Data.BLL
{
    public class GameDataBLL
    {
        ...
        public static PagedList<ScalePot> GetListByRoundForBaiJiaLe(GameRecordView model)
        {
            PagerQuery pq = new PagerQuery();
            pq.CurrentPage = model.Page;
            pq.PageSize = 10;
            pq.RecordCount = DAL.PagedListDAL<BaccaratGameRecord>.GetRecordCount(string.Format(@"select count(0) from " + database3 + @".BG_ScalePot where CreateTime between '{0}' and '{1}'  {2} ", model.StartDate, model.ExpirationDate, model.SearchExt == "" ? " " : " and  BoardNo = '" + model.SearchExt + "'"), sqlconnectionStringForRecord);
