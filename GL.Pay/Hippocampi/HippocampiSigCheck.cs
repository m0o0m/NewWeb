...
namespace GL.Pay.Hippocampi
{
    public class HippocampiSigCheck
    {
        public HippocampiSigCheck()
        {
        }
        ...
        static public string MakeSig( Dictionary<string, string> param, string secret)
        {
            ...
            log.Info(" 海马md5校验第三步：获得编码后的url组合参数:" + mk);
            //使用SHA1的HMAC
            ...
            log.Info(" 海马md5校验第四步：新的md5编码" + my_sign);
            return my_sign;
        }
        ...
        {
            ILog log = LogManager.GetLogger("Callback");
            log.Info(" 海马md5校验第一步：检测参数:"+ JsonConvert.SerializeObject(param)); 
            ...
            if (param.ContainsKey("user_param"))
            {
                param.Remove("user_param");
            }
            log.Info(" 海马md5校验第二步：检测过滤后的参数:" + JsonConvert.SerializeObject(param));
            ...
            log.Info(" 海马md5校验最后一步：对比签名,传过来签名：" + sign+" ;新生成签名: "+sig_new);
