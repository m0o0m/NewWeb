...
namespace GL.Data.DAL
{
    public class ManagerInfoDAL
    {
        ...
        internal static int UpdateState(ManagerInfo model)
        {
            using (var cn = new MySqlConnection(sqlconnectionString))
            {
                cn.Open();
                int i = cn.Execute(@"update ManagerInfo set AdminMasterRight = @AdminMasterRight where AdminID = @AdminID", model);
                cn.Close();
                return i;
            }
        }
