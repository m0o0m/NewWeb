    ...
    internal class PagerBuilder
    {
        ...
        public string GenerateHtml()
        {
            ...
            if ((_totalPageCount > 1 || (!_pagerOptions.AutoHide)) && ! _pagerOptions.HidePagerItems)
            {
                ...
                if (string.IsNullOrEmpty(_pagerOptions.WYWPageIndexString))
                {
                    tb.InnerHtml = sb.ToString();
                }
                else {
                    tb.InnerHtml = sb.ToString() + string.Format(_pagerOptions.WYWPageIndexString, _pagerOptions.PageIndexBoxId, _pagerOptions.PageIndexBoxId);
                }
